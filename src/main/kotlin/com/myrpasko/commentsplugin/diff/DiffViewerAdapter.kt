...
object DiffViewerAdapters {
    ...
    private fun createReflectiveAdapter(
        ...
        val changeList = diffChangesMethod.invoke(viewer) as? Iterable<*> ?: return null
        val firstChange = changeList.firstOrNull()
        val changedLines = if (firstChange == null) {
            emptySet()
        } else {
            val accessors = ReflectiveChangeAccessors.from(firstChange) ?: return null
            buildReflectiveChangedLines(changeList, accessors)
        }
        ...
        return AdapterImpl(
            ...
            primaryComponent = viewer.component,
            viewerKind = viewer.javaClass.name,
            changedLinesProvider = { changedLines },
        )
    }
